...
const Comment = (props) => {
  ...
  return (
    <div className={classes.root}>
        ...
        <Typography className={classes.userName} color="primary"> {user.username}</Typography>
            <div className={classes.comment2}> {props.comment.content} </div> 
            {
           localStorage.getItem("user_id") == props.comment.user_id ?
          <Button className={classes.button} onClick={() => props.deleteComment(props.comment)} size="small" color="secondary">
            Delete
          </Button>
          :
          null
